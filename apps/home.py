...
def add_selectors(farms):
    ...
    farm_names_col, indices_list_col, max_cloud_cover_col = st.columns([3, 3, 3])

    with farm_names_col:
        selected_name = st.selectbox(
            "Select the farm to monitor",
            farm_names,
            index=farm_names.index("ADM Farm"),
        )
    ...
    with indices_list_col:
        selected_index = st.selectbox(
            ...
            indices_list,
            index=indices_list.index("Crop Health"),
        )
    ...
    with max_cloud_cover_col:
        ...
        return farm_names_col, indices_list_col, max_cloud_cover_col
